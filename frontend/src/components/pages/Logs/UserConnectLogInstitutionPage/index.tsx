...
import type { UserConnectLogInstitution } from '@/types'
...
export function UserConnectLogInstitutionPage() {
  const { user } = useUserStore()
  const defaultDates = useMemo(() => getDefaultDates(), [])

  const [dateInputs, setDateInputs] = useState(defaultDates)
  const [data, setData] = useState<UserConnectLogInstitution[]>([])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const result = await logsApi.getUserConnectLogInstitution(params)
      setData(result)
    } catch (err) {
        ...
        <table className="w-full border-collapse text-sm">
          <thead className="sticky top-0 bg-gray-100">
            <tr>
              ...
              <th className="border-b border-gray-300 p-2 text-center">접속건수</th>
            </tr>
          </thead>
          <tbody>
            {isLoading ? (
              ...
              data.map((item, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className="border-b border-gray-200 p-2 text-center">{item.instCd}</td>
                  <td className="border-b border-gray-200 p-2 text-center">{item.instNm}</td>
                  <td className="border-b border-gray-200 p-2 text-center">{item.connectCnt}</td>
                </tr>
              ))
            )}
          </tbody>
