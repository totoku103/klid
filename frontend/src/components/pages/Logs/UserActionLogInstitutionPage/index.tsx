...
import type { UserActionLogInstitution } from '@/types'
...
export function UserActionLogInstitutionPage() {
  const { user } = useUserStore()
  const defaultDates = useMemo(() => getDefaultDates(), [])

  const [dateInputs, setDateInputs] = useState(defaultDates)
  const [data, setData] = useState<UserActionLogInstitution[]>([])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const result = await logsApi.getUserActionLogInstitution(params)
      setData(result)
    } catch (err) {
