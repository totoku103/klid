...
import type { UserActionLogPeriod } from '@/types'
...
export function UserActionLogPeriodPage() {
  const { user } = useUserStore()
  const defaultDates = useMemo(() => getDefaultDates(), [])

  const [dateInputs, setDateInputs] = useState(defaultDates)
  const [data, setData] = useState<UserActionLogPeriod[]>([])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const result = await logsApi.getUserActionLogPeriod(params)
      setData(result)
    } catch (err) {
