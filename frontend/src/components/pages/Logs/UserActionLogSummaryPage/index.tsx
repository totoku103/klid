...
import type { UserActionLogSummary } from '@/types'
...
export function UserActionLogSummaryPage() {
  const { user } = useUserStore()
  const defaultDates = useMemo(() => getDefaultDates(), [])

  const [dateInputs, setDateInputs] = useState(defaultDates)
  const [data, setData] = useState<UserActionLogSummary[]>([])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const result = await logsApi.getUserActionLogSummary(params)
      setData(result)
    } catch (err) {
        ...
        <table className="w-full border-collapse text-sm">
          <thead className="sticky top-0 bg-gray-100">
            <tr>
              ...
              <th className="border-b border-gray-300 p-2 text-center">활동유형</th>
            ...
            {isLoading ? (
              ...
              data.map((item, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className="border-b border-gray-200 p-2 text-center">{item.actionType}</td>
