...
export function UserInoutHistPage() {
  ...
  const [data, setData] = useState<UserInoutHist[]>([])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const result = await histApi.getUserInoutHistList(params)
      setData(result)
    } catch (err) {
        ...
        <table className="w-full border-collapse text-sm">
          <thead className="sticky top-0 bg-gray-100">
            <tr>
              ...
              <th className="border-b border-gray-300 p-2 text-center">브라우저</th>
            </tr>
          </thead>
          <tbody>
            {isLoading ? (
              ...
              data.map((item, idx) => (
                <tr key={item.histSeq} className="hover:bg-gray-50">
                  ...
                  <td className="border-b border-gray-200 p-2 text-center">{item.inoutType}</td>
                  <td className="border-b border-gray-200 p-2 text-center">{item.inoutDt}</td>
                  <td className="border-b border-gray-200 p-2 text-center">{item.ipAddr}</td>
                  <td className="border-b border-gray-200 p-2 text-center">{item.browser}</td>
                </tr>
              ))
            )}
          </tbody>
