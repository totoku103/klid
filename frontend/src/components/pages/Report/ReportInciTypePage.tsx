...
export function ReportInciTypePage() {
  ...
  const [data, setData] = useState<InciTypeReport[]>([])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const result = await rptApi.getInciTypeList(params)
      setData(result)
    } catch (err) {
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              <>
                {data.map((item, idx) => (
                  <tr key={item.typeCode} className="hover:bg-gray-50">
