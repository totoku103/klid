...
import type { InciPrtyReport } from '@/types'
...
export function ReportInciPrtyPage() {
  ...
  const [data, setData] = useState<InciPrtyReport[]>([])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const result = await rptApi.getInciPrtyList(params)
      setData(result)
    } catch (err) {
  ...
  return (
    <SubPageLayout locationPath={['보고서', '우선순위별 통계']}>
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              <>
                {data.map((item, idx) => (
                  <tr key={item.prtyCd} className="hover:bg-gray-50">
