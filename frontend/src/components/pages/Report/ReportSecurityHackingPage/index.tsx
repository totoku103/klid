...
import type { SecurityHackingReport } from '@/types'
...
export function ReportSecurityHackingPage() {
  const { user } = useUserStore()
  const defaultDates = useMemo(() => getDefaultDates(), [])

  const [dateInputs, setDateInputs] = useState(defaultDates)
  const [data, setData] = useState<SecurityHackingReport[]>([])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const result = await rptApi.getSecurityHackingList(params)
      setData(result)
    } catch (err) {
  ...
  return (
    <SubPageLayout locationPath={['보고서', '보안해킹현황']}>
      ...
      <div className="h-[calc(100%-130px)] overflow-auto rounded border border-gray-300">
        <table className="w-full border-collapse text-sm">
          <thead className="sticky top-0 bg-gray-100">
            <tr>
              <th className="border-b border-gray-300 p-2 text-center">해킹유형</th>
              ...
              <>
                {data.map((item, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    <td className="border-b border-gray-200 p-2 text-center">{item.hackingType}</td>
                ...
                <tr className="bg-gray-50 font-semibold">
                  <td className="border-b border-gray-300 p-2 text-center">합계</td>
