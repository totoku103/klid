...
import type { InciPrcsStatReport } from '@/types'
...
export function ReportInciPrcsStatPage() {
  ...
  const [data, setData] = useState<InciPrcsStatReport[]>([])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const result = await rptApi.getInciPrcsStatList(params)
      setData(result)
    } catch (err) {
  ...
  return (
    <SubPageLayout locationPath={['보고서', '처리상태별 통계']}>
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              <>
                {data.map((item, idx) => (
                  <tr key={item.statCd} className="hover:bg-gray-50">
