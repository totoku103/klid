...
import type { InciAttNatnReport } from '@/types'
...
export function ReportInciAttNatnPage() {
  ...
  const [data, setData] = useState<InciAttNatnReport[]>([])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const result = await rptApi.getInciAttNatnList(params)
      setData(result)
    } catch (err) {
  ...
  return (
    <SubPageLayout locationPath={['보고서', '공격국가별 통계']}>
        ...
        <table className="w-full border-collapse text-sm">
          <thead className="sticky top-0 bg-gray-100">
            <tr>
              ...
              <th className="border-b border-gray-300 p-2 text-center">국가</th>
          ...
          <tbody>
            {isLoading ? (
              ...
              <>
                {data.map((item, idx) => (
                  <tr key={item.nationCd} className="hover:bg-gray-50">
