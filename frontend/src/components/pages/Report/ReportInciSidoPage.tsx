...
import type { InciSidoReport } from '@/types'
...
export function ReportInciSidoPage() {
  ...
  const [data, setData] = useState<InciSidoReport[]>([])
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const result = await rptApi.getInciSidoList(params)
      setData(result)
    } catch (err) {
  ...
  return (
    <SubPageLayout locationPath={['보고서', '시도별 통계']}>
        ...
        <table className="w-full border-collapse text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              <>
                {data.map((item, idx) => (
                  <tr key={item.sidoCd} className="hover:bg-gray-50">
